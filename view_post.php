...
<body>
    ...
    <section class="view-post">
        ...
        <?php
        $select_post = $db->prepare("SELECT * FROM `participant` WHERE id = ? LIMIT 1");
        ...
        if ($select_post->rowCount() > 0) {
            while ($fetch_post = $select_post->fetch(PDO::FETCH_ASSOC)) {
                ...
                $select_ratings = $db->prepare("SELECT * FROM `reviews` WHERE participant_id = ?");
    ...
    <section class="reviews-container">
        ...
        <div class="box-container">
            ...
            <?php
            $select_reviews = $db->prepare("SELECT * FROM `reviews` WHERE participant_id = ?");
            $select_reviews->execute([$get_id]);
            if ($select_reviews->rowCount() > 0) {
                while ($fetch_review = $select_reviews->fetch(PDO::FETCH_ASSOC)) {
            ?>
            <div class="box" <?php if ($fetch_review['jury_id'] == $jury) {
                                    echo 'style="order: -1;"';
                                } ?>>
                <?php
                        $select_jury = $db->prepare("SELECT * FROM `jury` WHERE id = ?");
                        $select_jury->execute([$fetch_review['jury_id']]);
                        while ($fetch_jury = $select_jury->fetch(PDO::FETCH_ASSOC)) {
                    ?>
                     <div class="user">  
                    <h3><?= substr($fetch_jury['name'], 0, 1); ?></h3>  
                        <div>
